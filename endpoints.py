...
def create_temporary_session(request: Request) -> SessionCreateResponse:
    ...
    if not check_rate_limit(client_ip):
        raise HTTPException(
            status_code=429, detail="Rate limit exceeded. Too many session requests."
        )
...
def get_session_status(session_id: str) -> SessionStatusResponse:
    ...
    if session_id not in session_registry:
        return SessionStatusResponse(user_id=session_id, is_valid=False)
...
def cleanup_session(session_id: str, request: Request) -> dict:
    ...
    if authenticated_user_id == session_id or session_id in session_registry:
        ...
        raise HTTPException(
            status_code=403, detail="Invalid authentication for session cleanup"
        )
...
# ---------------------------------------------------------------------------
# Chatbot frontend - served as static files from the bundled frontend/ folder
# ---------------------------------------------------------------------------


@endpoint.get("/chatbot", tags=["Temp Chat Auth"])
def serve_chatbot_ui():
    """Serve the embedded chatbot HTML page with plugin settings injected."""
        ...
        + _json.dumps(
            {
                "headerTitle": settings.get(
                    "chatbot_header_title", "Virtual Assistant"
                ),
...
def serve_chatbot_asset(filename: str):
    """Serve chatbot static assets (images, fonts, etc.)."""
    asset_path = os.path.join(FRONTEND_DIR, "assets", filename)
    # Guard against path-traversal
    if not os.path.isfile(asset_path) or not os.path.abspath(asset_path).startswith(
