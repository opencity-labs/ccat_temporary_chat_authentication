        ...
        if not settings["episodic_memory_for_tmp"]:
            try:
                ...
                episodic_collection.delete_points_by_metadata_filter(
                    {"skip_storage": True, "source": cat.user_id}
                )
                ...
                episodic_collection.delete_points_by_metadata_filter(
                    {"source": cat.user_id, "session_type": "temporary"}
                )
...
def after_cat_bootstrap(cat):
    ...
    if settings["auto_configure_auth"]:
        try:
            ...
            if (
                selected_auth is None
                or selected_auth.get("value", {}).get("name")
                != "TemporarySessionAuthConfig"
            ):
